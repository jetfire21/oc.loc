	...
	public function addTransaction($affiliate_id, $description = '', $amount = '', $order_id = 0, $firstname, $lastname) {
		...
		if ($affiliate_info) { 
			...
			if($affiliate_id!=0) {
		        ...
		        $balans = $this->model_module_affiliatemmm->getBalansAff((int)$parentaffiliate['affiliate_id']);
				$ostatok = (float)$parentaffiliate['total'] + $balans;

          if ((int)$order_id != 0 & (float)$parentaffiliate['total'] != 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "affiliate_transaction` SET affiliate_id = '" . (int)$parentaffiliate['affiliate_id'] . "', order_id = '" . (float)$order_id . "',operation = 'Оплата заказа №" . (float)$order_id. "',payment='1', name='".$affiliate_info['firstname'] ." ". $affiliate_info['lastname']."', description = '" . $this->db->escape($description). " (" .$affiliate_info['firstname'] ." ". $affiliate_info['lastname'] . ")', ostatok = '" . (float)$ostatok . "', amount = '" . (float)$parentaffiliate['total'] . "', date_added = NOW(), affiliate_children = '" . $affiliate_id . "'");

            $query = $this->db->query("SELECT balans FROM " . DB_PREFIX . "affiliate WHERE affiliate_id = '" . (int)$parentaffiliate['affiliate_id'] . "'");
			$balans = (float)$parentaffiliate['total'] + $query->row['balans'];
            $this->db->query("UPDATE " . DB_PREFIX . "affiliate SET balans =". $balans ." WHERE affiliate_id = '" . (int)$parentaffiliate['affiliate_id'] . "'");
          } 
        }
			}
