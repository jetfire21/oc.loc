...
class ModelCatalogProduct extends Model {
	...
	public function MYgetProducts(){
		$sql = "SELECT a.*,a.price roz_price,b.*,c.price special,c.date_end FROM " . DB_PREFIX . "product a 
	LEFT JOIN " . DB_PREFIX . "product_description b ON b.product_id = a.product_id 
	LEFT JOIN " . DB_PREFIX . "product_special c ON c.product_id = a.product_id
	WHERE a.hot = '1'";
	...
	public function getProductsTopAkses(){
		$sql = "SELECT a.*,a.price roz_price,b.*,c.price special,c.date_end FROM " . DB_PREFIX . "product a 
	LEFT JOIN " . DB_PREFIX . "product_description b ON b.product_id = a.product_id 
	LEFT JOIN " . DB_PREFIX . "product_special c ON c.product_id = a.product_id
	WHERE a.top_akses = '1'";
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < '" . $this->NOW . "') AND (pd2.date_end = '0000-00-00' OR pd2.date_end > '" . $this->NOW . "')) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "')) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, 
			(SELECT date_end FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->NOW . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->NOW . "')) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special_end,
