...
class ControllerProductCategory extends Controller {  
	public function index() { 
		...
		if (isset($this->request->get['path'])) {
			...
			// 
			$my_path = (string)$this->request->get['path'];
			$this->data['subcat_id'] = $category_id;
			// если сущ _ в path(23_34)
			if (preg_match("/_/i", $my_path)) $parent_cat_id = $parts[0];
		...
		if ($category_info) {
			...
			$this->data['$all_cat'] = array();

			if($parent_cat_id){
				$results2 = $this->model_catalog_category->getCategories($parent_cat_id);
				...
				foreach ($results2 as $result) {
					...
					$parts2 = explode('_', (string)$this->request->get['path']);	
				    $par_id = (int)$parts2[0];

					if (preg_match("/_/i", (string)$this->request->get['path'] ))	{
						
						$href  = $this->url->link('product/category', 'path=' . $par_id . '_' . $result['category_id'] . $url);
